...
def run_result():
    with op.DBManger() as m:
        ...
        last_sep = m.get_lasted_collection("jingdong", filter={"name": {"$regex": r"^jdprice20201031_sep"}})
        for table in m.list_tables(dbname="jingdong",filter={"name": {"$regex": r"^jdprice(20201031)$"}}):
        ...
        last_sep = m.get_lasted_collection("jingdong", filter={"name": {"$regex": r"^jdskuid20200920_sep"}})
        for table in m.list_tables(dbname="jingdong",filter={"name": {"$regex": r"^jdskuid(20200920)retry\d*$"}}):
            if not last_sep or table > last_sep:
        ...
        last_sep = m.get_lasted_collection("jingdong", filter={"name": {"$regex": r"^jdcomment20201204_sep"}})
        for table in m.list_tables(dbname="jingdong",filter={"name": {"$regex": r"^jdcomment(20201204)retry\d*$"}}):
            if not last_sep or table > last_sep:
