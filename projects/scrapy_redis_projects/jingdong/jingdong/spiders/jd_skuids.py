...
class Spider(JiChengSpider):
    ...
    def parse3(self, response):
        ...
        shopid = self.shopid_pettern.findall(response.text)[0]
        if shopid:
            shopid = shopid[0]
        else:
            shopid = None
        venderid = self.venderid_pettern.findall(response.text)[0]
        if venderid:
            venderid = venderid[0]
        else:
            venderid = None
                ...
                for pid in r1.split(","):
                    if brand_id:
                        ...
                        request.headers[
                            "Referer"] = "https://list.jd.com/list.html?{0}&page={1}&s={2}&psort=4&click=1".format(
                            en_cate_id, page, s)
                    request.meta["_seed"] = (cate_id, brand_id, page, s)
                    request.dont_filter = True
                    print(request)
                    yield request
    ...
    def parse(self, response):
        ...
        if page_strs:
            ...
            for i in range(1, int(page_strs) + 1):
                ...
                yield Request(url=url, callback=self.parse1, meta={"_seed":(cate_id, brand_id, page, s), "headers": {"Connection": "close", "Referer": refer}}, priority=1, dont_filter=True)
    ...
    def init_start_urls(self):
        ...
        with op.DBManger() as m:
            ...
            for i, seed in enumerate(data_set.distinct()):
                ...
                if not (cate_id == "4938,11760,12282" and brand_id == "7575"):
                    continue
                if brand_id:
