...
class Spider(JiChengSpider):
    ...
    def make_request_from_data(self, data):
        ...
        if seed.type == 0:
            ...
            url = "https://club.jd.com/comment/skuProductPageComments.action?callback=fetchJSON_comment98" \
                  "&productId={0}&score=0&sortType=6&page=0&pageSize=10&isShadowSku=0&fold=1".format(skuid)
    ...
    def parse(self, response):
        ...
        for item in json.loads(self.comments_pattern.findall(response.text)[0]):
        ...
        for pindex in range(maxpagesindex+1):
            url = "https://club.jd.com/comment/skuProductPageComments.action?callback=fetchJSON_comment98" \
                  "&productId={0}&score=0&sortType=6&page={1}&pageSize=10&isShadowSku=0&fold=1".format(skuid, pindex)
            yield Request(url=url, meta={"_seed": response.meta["_seed"],
    ...
    def parse1(self, response):
        for item in json.loads(self.comments_pattern.findall(response.text)[0]):
            yield {"id": item.get("id"), "creationTime": item.get("creationTime"),
                   "discussionId": item.get("discussionId"), "referenceId": item.get("referenceId"),
                   "referenceTime": item.get("referenceTime"), "nickname": item.get("nickname"),"userClient":item.get("userClient")}
...
class FirstMaster(Master):
    ...
    def init_start_urls(self):
        ...
        #         random.shuffle(buffer)
        #         self.redis.sadd(self.start_urls_redis_key, *buffer)
        self.redis.sadd(self.start_urls_redis_key, *[str(Seed(value=65402777116, type=0))])
    ...
    def get_thread_writer(self):
        thread_writer = ThreadMongoWriter(redis_key=self.items_redis_key, stop_epoch=12*3000,buffer_size=2048,
                                          ...
                                          db_collection=("jingdong","jdcommentdetail_v2{0}retry0".format(current_date)), bar_name=self.items_redis_key, distinct_field=None)
