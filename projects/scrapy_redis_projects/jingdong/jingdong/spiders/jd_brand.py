...
class Spider(JiChengSpider):
    ...
    def parse(self, response):
        ...
        if len(tuples) > 0:
            for item in tuples:
                yield {"brand_id": item[0], "name": item[1], "cate_id": seed.value, "_seed": seed.value, "status": 0}
        elif response.text.find("""<span class="result">抱歉，没有找到与“<em></em>”相关的商品</span>""") == -1:
            #没有品牌的分类
            yield {"cate_id": seed.value, "_seed": seed.value, "status": -1}
        else:
            #不存在的分类
            yield {"cate_id": seed.value, "_seed": seed.value, "status": -2}
...
class FirstMaster(Master):
    ...
    def get_thread_writer(self):
        thread_writer = ThreadMongoWriter(redis_key=self.items_redis_key, stop_epoch=12*3000,buffer_size=2048,
                                          ...
                                          db_collection=("jingdong","jdbrand{0}retry0".format(current_date)), bar_name=self.items_redis_key, distinct_field=None)
...
class RetryMaster(FirstMaster):
    def __init__(self, *args, **kwargs):
        ...
        with op.DBManger() as m:
            self.last_retry_collect = m.get_lasted_collection("jingdong", filter={"name": {"$regex": r"^jdbrand20\d\d\d\d\d\d(retry\d+)?$"}})
