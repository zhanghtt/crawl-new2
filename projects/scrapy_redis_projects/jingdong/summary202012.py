...
def run_result():
    with op.DBManger() as m:
        ...
        for table in m.list_tables(dbname="jingdong",filter={"name": {"$regex": r"^jdprice(20210109)$"}}):
        ...
        for table in m.list_tables(dbname="jingdong",filter={"name": {"$regex": r"^jdcomment(20210111)retry\d*$"}}):
            if not last_sep or table > last_sep:
