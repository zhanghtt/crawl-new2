...
if __name__=='__main__':
    ...
    with op.DBManger() as m:
        ...
        month = '202012'
        tables = m.list_tables(dbname="jingdong",filter={"name": {"$regex": r"^jdprice20210109$"}})
        month = '202009'
        tables = m.list_tables(dbname="jingdong", filter={"name": {"$regex": r"^jdprice20200929$"}})
        #tables.extend(m.list_tables(dbname="jingdong",filter={"name": {"$regex": r"^jdprice20210302$$"}}))
        schema = StructType([StructField("skuid", LongType(), True),StructField("price", DoubleType(), True),StructField("month", StringType(), True)])
        ...
        for table in tables:
            ...
            tmp = tmp.withColumn("price", clean_price(array('p','op','cbf','l','m','op','p','tpp','up'))).withColumn('skuid',F.col('id').cast(LongType())).select('skuid','price').withColumn('month', F.lit(month))
            df = df.unionAll(tmp)
        ...
        tmp = spark.read.format("mongo").option("uri", "mongodb://192.168.0.13:27017/jingdong.dyf_comment").option(
            ...
            [StructField("skuid", LongType(), True), StructField("comment", IntegerType(), True), StructField("month", StringType(), True)])).load().filter("month={}".format(month))
