...
if __name__=='__main__':
    ...
    with op.DBManger() as m:
        ...
        month = '202012'
        tables = m.list_tables(dbname="jingdong", filter={"name": {"$regex": r"^jdcomment(20210111)retry\d*$"}})
        month = '202009'
        tables = m.list_tables(dbname="jingdong", filter={"name": {"$regex": r"^jdcomment(20200923)retry\d*$"}})
        #tables.extend(m.list_tables(dbname="jingdong",filter={"name": {"$regex": r"^jdcomment(20210316)retry\d*$"}}))
        df = spark.createDataFrame([], StructType([StructField("skuid", LongType(), True),StructField("comment", IntegerType(), True),StructField("month", StringType(), True)]))
        for table in tables:
            tmp = spark.read.format("mongo").option("uri","mongodb://192.168.0.13:27017/jingdong.{}".format(table)).\
                option("spark.mongodb.input.partitioner","MongoSplitVectorPartitioner").schema(StructType([StructField("skuid", LongType(), True),StructField("comment", StringType(), True)])).load().withColumn('comment',F.col('comment').cast(IntegerType())).filter("comment > 0").select(['skuid','comment']).withColumn('month', F.lit(month))
            df = df.unionAll(tmp)
        df = df.groupBy(['skuid','month']).agg(F.max('comment').alias('comment'))
        dyf_skuid = spark.read.format("mongo").option("uri","mongodb://192.168.0.13:27017/jingdong.dyf_skuid").option("spark.mongodb.input.partitioner","MongoSplitVectorPartitioner").schema(StructType([StructField("skuid", LongType(), True)])).load().withColumn('month', F.lit(month)).distinct()
