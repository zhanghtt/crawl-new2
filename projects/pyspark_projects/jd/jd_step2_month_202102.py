...
if __name__=='__main__':
    ...
    with op.DBManger() as m:
        month = '202102'
        last_month = '202101'
        tables = m.list_tables(dbname="jingdong", filter={"name": {"$regex": r"^jdpricejdprice20210222$"}})
        ...
        df = df.join(df_price,['skuid'],'left').withColumn("clean_price", F.when(F.isnull(df_price.price),df.clean_price).otherwise(df_price.price)).select(['skuid','comments','clean_price'])
