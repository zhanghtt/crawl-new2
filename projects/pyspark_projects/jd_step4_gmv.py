...
if __name__=='__main__':
    ...
    with op.DBManger() as m:
        tables = m.list_tables(dbname="jingdong",filter={"name": {"$regex": r"^jdprice20210301$"}})
        #tables.extend(m.list_tables(dbname="jingdong",filter={"name": {"$regex": r"^jdprice20210302$$"}}))
        schema = StructType([StructField("id", IntegerType(), True),StructField("price", DoubleType(), True),StructField("month", StringType(), True)])
        ...
        for table in tables:
            ...
            tmp = tmp.withColumn("price", clean_price(array('p','op','cbf','l','m','op','p','tpp','up'))).withColumn('skuid',F.col('id').cast(IntegerType())).select('skuid','price').withColumn('month', F.lit('202103'))
            df = df.unionAll(tmp)
        df = df.distinct()
        ...
        df = df.join(tmp, [df.month == tmp.month,df.skuid == tmp.skuid], 'right').select([df.month, tmp.skuid, tmp.comment, df.price])
